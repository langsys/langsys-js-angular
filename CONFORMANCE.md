...
| **OBS-1** — surface an unusable capability at least once                                           | `partial` (gap) | The binding emits no diagnostic. It does expose `error`, but nothing fires when authorization resolves `write_enabled: false` on a key expected to write. Whether the diagnostic is the core's or the binding's is unsettled; listed so it is countable rather than invisible.                                                                                                                                                           |
...
1. **OBS-1 has no diagnostic.** Now the top gap. A misconfigured integration — a write key that is
   read-only from this address — is silent unless the app thinks to watch `writeEnabled`. Blocked
   on an owner ruling: core or binding.
2. **The pipe memo question is still open** (BIND-5 / GATE-2 / GATE-7 / CAT-1). Routed to the
   Typescript lane; the pipe was deliberately left untouched pending the ruling.
3. **The hydration guard has never met a real hydration.** jsdom proves the deferral and the
   ...
   and flagged for the mandatory review this work was promised.
4. **The service spec's `canonicalizeLocale` double disagrees with the real core.** It uppercases
   the region (`en-US`); the real implementation lowercases (`en-us`, WIRE-3). No test currently
   depends on the difference, but a double that contradicts production can certify the wrong
   behaviour later. Filed rather than fixed — changing it is a separate, testable change.
